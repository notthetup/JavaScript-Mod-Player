...
function schedSound(source, when_secs, pitch, tickspersample, volume, volumechange, samplenum, modfile, state) {

  if (state !== "init"){
    console.warn("trying to schedule sound that's not yet init");
    return;
  }
    ...
    for (var j = 0; j < 64; j++) {
      for( var k=0;k< modfile.channelNum; k++){
        ...
        if(currentChannels[k].sample!=0){
          if(j!=0){
            if (sourcesState[c] === "playing"){
              sources[k].stop(m_time + i * 7.68 + j * 0.12);
              sourcesState[c] = "stopped";
            }else{
                console.log(c,"wasn't playing");
            }
          }
          sources[k] = context.createBufferSource();
          sourcesState[k] = "init";
          schedSound(sources[k], m_time + i * 7.68 + j * 0.12, currentChannels[k].pitch, ticksperSample, currentChannels[k].volume, currentChannels[k].volumechange, currentChannels[k].sample - 1, modfile, sourcesState[k]);
        }
      }
